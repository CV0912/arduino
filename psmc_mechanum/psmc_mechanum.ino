...
void compute() {
  ...
  float s1 = c * error1+ derr1;
  float s2 = c * error2+ derr2;
  float s3 = c * error3+ derr3;
  output = kp * error + kd * derr + kt * (k * sign(s));
  output1 = kp * error1 + kd * derr1 + kt * (k * sign(s1));
  output2 = kp * error2 + kd * derr2 + kt * (k * sign(s2));
  output3 = kp * error3 + kd * derr3 + kt * (k * sign(s2));
